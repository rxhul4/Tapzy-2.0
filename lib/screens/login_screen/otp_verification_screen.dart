...
class OtpVerificationScreen extends StatefulWidget {
  ...
  const OtpVerificationScreen(
      {Key? key, required this.email, required this.isNew})
      : super(key: key);
...
class _OtpVerificationScreenState extends State<OtpVerificationScreen>
  ...
  Future<void> _callOtpVerifyApi(LoginProvider provider) async {
    final value =
        await provider.callOtpVerifyApi(otpValue ?? '', widget.email);
    ...
    if (otpVerifyModel?.isSuccessful == 1) {
      Navigator.pushAndRemoveUntil(
          ...
          MaterialPageRoute(
              builder: (_) =>
                  widget.isNew == 1 ? UserDetailScreen() : DashboardScreen()),
          (r) => false);
    } else {
      AppUtils.showSnackBarWithColor(
          context: context,
                      ...
                      children: [
                        ...
                        GlassContainer(
                          ...
                          pinTheme: PinTheme(
                            ...
                            selectedColor: AppColors.colorPurple,
                          ),
                          onChanged: (v) => setState(() {
                            otpValue = v;
                            if (v.isNotEmpty) errorText = '';
                          }),
                          onCompleted: (_) => _callOtpVerifyApi(provider),
                        ),
                        ),
                        ...
                        GlassUi.primaryButton(
                          label: 'Verify OTP',
                          isLoading:
                              provider.isFetching || provider.isLoading,
                          onTap: () {
